...
Bei der Datenaufbereitung für Fussgänger-Routing über Strassen ergeben sich andere Anforderungen als das Routing für den Strassenverkehr. So müssen etwa Bürgersteige und Fussgänger-Streifen beachtet werden, um zu entscheiden, ob eine Strasse für Fussgänger begehbar ist.
...
In einem zweiten Teil werden die im Teil 1 erarbeiteten Artefakte in einem Prototyp als QGIS-Plugin zusammengeführt. Im QGIS-Plugin wird ein multimodales Routing visuell dargestellt. So ist ein optimiertes Fussgänger-Routing in Kombination mit dem öffentlichen Verkehr möglich.

Die Ergebnisse werden im Kapitel \ref{chap:SW-Projektdokumentation} beschrieben.
...
Auf Flächen können Hindernisse existieren, durch welche nicht geroutet werden darf. Dazu gehören selbstredend Häuser und dergleichen aber auch Barrieren gekennzeichnet mit dem Attribut \textit{barrier=*} \cite{osm_wiki_barrier}, was beispielsweise ein Mauer oder Hecke beschreiben kann.

\subsubsection{ÖV-Haltestelle und Kante}
\label{ÖV-Haltestelle und Kante}
Eine ÖV-Haltestelle kann mehrere Stationen umfassen. Normalerweise gehören zu einer Haltestelle zwei Stationen, je eine in jede Fahrrichtung. Eine dieser Station wird im folgenden als Kante bezeichnet. Der Unterschied ist in Abbildung \ref{fig:public_transport_stop} ersichtlich. Eine ÖV-Haltestelle beschränkt sich dabei nicht auf zwei Kanten. Betrachtet man den Bahnhof Stadelhofen in Zürich, sieht man das weit mehr als zwei Kanten zu einer ÖV-Haltestelle gehören können.
