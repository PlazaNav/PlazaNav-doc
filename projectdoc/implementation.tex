...
%TODO Plugin Builder, Plugin Repo erwähnen
Das QGIS Plugin Plaza Route fügt sich nahtlos als \emph{Dockwidget} in die QGIS-Umgebung ein. In Abbildung \ref{fig:qgis_plugin_plaza_route} sieht man dies in Aktion. Beim Entwicklen des Plugin wurde Wert darauf gelegt, dass man Funktionalität bietet, welche bei vergleichbaren Produkten verfügbar sind. So lassen sich die Start und die Destination über eine Kontextmenu direkt auf der Karte auswählen. Das Plugin lässt Koordinaten, sowie Adressen zu. Die Adresse können in einem beliebigen System, welches Postgis kennt, angeben werden. Dies ermöglicht eine unkomplizierte Anwendung, da das Plugin keine Anforderungen an das System stellt. Das Plugin wird installiert, eine Karte wird geöffnet und das Routing kann bereits durchgeführt werden.
...
% verschiede Koordinatensysteme, Flugverbindung, Unterschied precise public transport stops mit Screenshots
